...
const DETAILS = [
  { key: "verifyChannel", iconPath: ICON_PATHS.verifiedUser },
  { key: "cadence", iconPath: ICON_PATHS.timer },
  { key: "window", iconPath: ICON_PATHS.notificationsActive },
  { key: "reply", iconPath: ICON_PATHS.checkCircle },
  { key: "travel", iconPath: ICON_PATHS.globe },
  { key: "visible", iconPath: ICON_PATHS.visibility },
];
  ...
  return (
    <div className="max-w-screen-md mx-auto space-y-12">
      ...
      <nav className="flex items-center gap-2 text-label-md text-on-surface-variant">
        ...
        <span className="text-primary">{t("lifeCheck.title")}</span>
      </nav>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {DETAILS.map((detail) => (
            <Detail
              ...
              title={t(`lifeCheck.details.${detail.key}.title`)}
              body={t(`lifeCheck.details.${detail.key}.body`)}
            />
          ))}
        </div>
      </section>
      ...
      <section className="bg-surface-container-low rounded-2xl p-8 ghost-border space-y-4">
        <h2 className="text-headline-sm text-primary">
          {t("lifeCheck.afterInactivity.heading")}
        </h2>
        <p className="text-body-md text-on-surface-variant">
          {t("lifeCheck.afterInactivity.body")}
        </p>
        <Link
          href="/docs/recovery"
          className="inline-flex items-center gap-2 text-secondary font-bold text-body-md hover:underline"
        >
          {t("lifeCheck.afterInactivity.readMore")}
          <Icon path={ICON_PATHS.chevronRight} className="w-3 h-3" />
        </Link>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          <Icon path={ICON_PATHS.heartbeat} className="w-5 h-5 text-secondary" />
          <span className="text-body-md font-medium text-primary">
            {t("lifeCheck.goToLifeCheck")}
          </span>
        </Link>
        <Link
        ...
        >
          ...
          <span className="text-body-md font-medium text-primary">
            {t("lifeCheck.trustedContactsLink")}
          </span>
        </Link>
      </section>
    </div>

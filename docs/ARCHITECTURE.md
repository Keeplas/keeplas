...
flowchart LR
  ...
  user -->|HTTPS| edge["Request middleware<br/>(apps/web/src/start.ts)"]
  edge -->|signed audit ctx| next["TanStack Start SPA<br/>(apps/web)"]
...
| Package           | Role                                                                              | Notes                                                                                                                        |
| ----------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `apps/web`        | TanStack Start (Vite). PWA-ready. The only deployed surface.                      | Hosts all UI, auth flows, vault interactions. Talks to Convex via `ConvexProvider`.                                          |
...
│   • React 19 + TanStack Start   │   WebSocket    │  (convex.cloud or self-   │
...
Three components, two processes (browser + Vite server), one external service (Convex). The Vite dev server runs either on your **host** (native path, `pnpm dev`) or inside the **Docker container** (compose path, `docker compose up`). Convex always runs on Convex Cloud (unless `CONVEX_MODE=selfhosted`).

**Ports**:

| Port | Service               | Where it lives                        |
| ---- | --------------------- | ------------------------------------- |
| 3000 | Vite dev server (web) | host or Docker container              |
